...
func connect() *sql.DB {
	psqlInfo := fmt.Sprintf("host=%s port=$d user=%s password=%s sslmode=disable", host, port, user, password, dbname)
...
func Create(transaction transaction.Transaction) int {
	...
	var sqlStatement = `INSERT INTO transactions (id, title, amount, type,  created_at)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id;
	`
